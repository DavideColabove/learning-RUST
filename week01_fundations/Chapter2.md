...
Then we call the standard input 'stdin()' we use the method 'read_line()' on it passing the mutable reference of the 'guess' variabile.\\ The 'read_line()' method is accepting a &reference as parameter and outputting a 'io::Result' which is an enum containing Ok and Err: Err contains information about how or why the operation has failed.
